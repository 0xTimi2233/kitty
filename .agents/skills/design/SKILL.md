...
3. 写 `codexspec/runtime/runs/<run-id>/dispatch/architect-001.md`，包含 `Commit policy`。Architect 若更新 `codexspec/adr/*.md` 或 `codexspec/spec/*.md`，必须在最终报告前提交本轮变更。
...
6. 写 `codexspec/runtime/runs/<run-id>/dispatch/tester-001.md`，包含 `Commit policy`。Tester 若更新 `codexspec/spec/test-plan/*.md`，必须在最终报告前提交本轮变更。
...
10. 写 `codexspec/runtime/runs/<run-id>/dispatch/doc-reviewer-001.md`，包含 `Commit policy`。Doc Reviewer 只写 runtime review 产物时默认不提交；若 dispatch 允许其修改长期文档，必须先提交再返回。
