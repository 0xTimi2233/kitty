...
1. 调度 Architect 前，确认 `state.current_run` 存在、当前 phase 可进入 design、`dispatch-ledger.md` 和 planning package 已存在。否则停止并建议 `$spec:plan` 或 `$spec:resume`，不要修改 state。
...
7. 将 Architect 变更的 `codexspec/` 文档路径和报告作为 Tester allowed inputs。追加 Tester 调度行，调度 Tester，写入 runtime agent id，并在收到 Tester 回复后更新该行。
